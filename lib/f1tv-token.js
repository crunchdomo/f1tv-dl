...
const getF1tvToken = async (user, pass, debugMode = false) => {
    ...
    // First, try to auto-extract cookies from system browsers
    if (debug) console.log('🔍 Auto-extracting F1TV cookies from system browsers...');
    try {
        const autoToken = await autoExtractF1TVCookies(debug);
        if (autoToken) {
            if (debug) console.log('✅ Found F1TV cookie in system browser! Skipping login.');
            if (debug) console.log('💡 Using authentication token from browser cookies');
            return autoToken;
        }
    } catch (e) {
        if (debug) console.log(`❌ Auto-extraction failed: ${e.message}`);
    }
    
    // Second, try to load manually extracted cookies
    ...
    if (debug) console.log('❌ No cookies found, proceeding with automated login...');
    if (debug) console.log('💡 Tip: Login to F1TV in Chrome/Firefox, or run "node extract-cookies.js" for manual setup');
    ...
    const userDataDir = path.join(__dirname, '..', '.browser-sessions', 'f1tv-session');
    ...
    const browser = await puppeteer.launch({
        ...
        userDataDir: userDataDir, // Persistent browser session
        args: [
            ...
            '--disable-extensions-except-crashpad',
            '--disable-extensions',
    ...
    await page.click(usernameField); // Click to focus
    await page.waitForTimeout(getRandomInt(200, 500)); // Short delay after click
    
    // Type with random delays between characters
    for (const char of user) {
        await page.keyboard.type(char);
        await page.waitForTimeout(getRandomInt(50, 150)); // Human typing speed
    }
    ...
    await page.click(passwordField); // Click to focus
    await page.waitForTimeout(getRandomInt(200, 500)); // Short delay after click
    ...
    await page.waitForTimeout(getRandomInt(1500, 3000)); // Longer pause before submit
