...
const checkManualCookies = () => {
    const manualToken = loadManualCookies();
    if (manualToken) {
        if (process.env.F1TV_DEBUG === 'true') {
            console.log('✅ Found manual cookie! Using saved authentication token from .f1tv-cookies.json');
        }
        // Store the manual token in the regular token storage for consistency
        const ds = new DataStore(`${config.HOME}${config.PATH_SEP}${config.DS_FILENAME}`);
        ds.add('token', manualToken);
        return manualToken;
    }
    return null;
};
...
const saveF1tvToken = async (user, pass) => {
    const ds = new DataStore(`${config.HOME}${config.PATH_SEP}${config.DS_FILENAME}`);
    const debug = process.env.F1TV_DEBUG === 'true';
    
    // Use the new TokenManager for seamless authentication
    const tokenManager = new TokenManager(debug);
    const sessionToken = await tokenManager.getValidToken(user, pass);
